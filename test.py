...
from model_imae import VisionTransformer, eval
...
checkpoint_path = "../data/Vit_checkpoint/5/model_epoch_{epoch}.pth".format(epoch=args.load_epoch)
checkpoint = torch.load(checkpoint_path, map_location=device)  
model.load_state_dict(checkpoint["model"])
model.to(device)

rec_save_path = "../data/Vit_rec/rollout/{category}_{file_number}_r{rollout_times}".format(category=args.category, 
...
# valid_loss = eval(model, dataloader, mask_ratio,loss_fn, epoch, rec_save_path, device)
